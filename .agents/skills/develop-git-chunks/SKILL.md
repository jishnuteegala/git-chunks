...
5. For any change touching push, resume, retries, or remote state, verify end-to-end against a real local remote using the recipe in `docs/testing.md`: build the binary, create a scratch repository with a bare `remote.git` as `origin`, run the changed flow with `-p`, and inspect the bare repository with `git -C remote.git log`/`rev-list --count`. Use pre-receive hooks for failure injection or arrival recording as documented there. Never verify push behavior against a network remote. Completion: the smoke run's remote history matches the expected chunk sequence.
